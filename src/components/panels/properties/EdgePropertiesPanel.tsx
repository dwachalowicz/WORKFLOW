              ...
              {[
                ...
                return (
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className={`flex-1 h-8 text-xs gap-1 font-medium ${isActive ? 'bg-card shadow-sm hover:bg-card border border-border/50 ' + op.color : 'text-foreground/60 hover:text-foreground/80'}`}
                    onClick={() => {
                      if (isViewMode) return;
                      setEdges(eds => eds.map(ed => 
                        ed.id === activeEdge.id 
